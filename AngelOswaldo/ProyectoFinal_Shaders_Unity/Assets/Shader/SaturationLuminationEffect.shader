        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                fixed4 frag(v2f i) : SV_Target
                {
                    ...
                    fixed4 newCol = fixed4(lum, lum, lum, 1);
                    ...
                    if (RGB <= 0.5)
                    {
                        return lerp(col, _Color, lum);
                    }
                    else if (RGB > 0.5 && RGB <= 0.7)
                    {
                        return lerp(col, _NearColor, lum);
                    }
                    else if (RGB > 0.7 && RGB <= 0.8)
                    {
                        return lerp(col, _MediumColor, lum);
                    }
                    else if (RGB > 0.8 && RGB <= 0.9)
                    {
                        return lerp(col, _FarColor, lum);
                    }
                    else if (RGB > 0.9 && RGB <= 1)
                    {
                        return lerp(col, _LightColor, lum);
                    }
                    return newCol;
                }
                ENDCG
